            ...
            <div className="space-y-3">
                {actions.map((action) => {
                    ...
                    return (
                        ...
                        >
                            <div
                                className={`relative overflow-hidden rounded-xl border transition-all duration-300 ${isHovered
                                        ? "border-transparent bg-gradient-to-r p-[1px] " + action.gradient
                                        : "border-border/50 bg-transparent"
                                    } ${isClicked ? "opacity-60 scale-[0.98]" : ""}`}
                            >
                                <div className={`rounded-[11px] bg-card p-4 transition-all duration-300 ${isHovered ? "bg-card/95" : ""}`}>
                                    <div className="flex items-center gap-4">
                                        <div
                                            className={`relative w-12 h-12 rounded-xl flex items-center justify-center bg-gradient-to-br ${action.gradient} transition-all duration-300 group-hover:scale-110 group-hover:shadow-lg`}
                                            style={{
                                                boxShadow: isHovered
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between mb-0.5">
                                                <p className={`font-semibold transition-colors duration-300 ${isHovered ? "text-foreground" : "text-foreground/90"}`}>
                                                    {action.title}
                                                </p>
                                        ...
                                        <Icon
                                            ...
                                            className={`w-5 h-5 text-muted-foreground transition-all duration-300 ${isHovered ? "translate-x-1 opacity-100" : "opacity-0"}`}
                                        />
                                    </div>
                                </div>
                            </div>
